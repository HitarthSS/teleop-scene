...
OUT_NAME="${OUT_NAME:-thread_pad_resting_v1}"
...
echo "== Thread resting on deformable pad scene =="
...
PY
    ...
    python3 tools/export_thread_pad_resting_scene.py \
      ...
      --pad-margin 0.025 \
      --pad-min-half 0.055
  " 2>&1 | tee "$OUT_NAME/thread_on_pad_resting_log.txt"
...
echo "  $PROJECT_DIR/$OUT_NAME/thread_on_pad_resting_newton.usd"
echo "  $PROJECT_DIR/$OUT_NAME/thread_on_pad_resting_newton.obj"
echo "  $PROJECT_DIR/$OUT_NAME/thread_on_pad_resting_scene.npz"
echo "  $PROJECT_DIR/$OUT_NAME/thread_on_pad_resting_report.txt"
echo "Archive:"
