...
OUT_NAME="${OUT_NAME:-thread_robot_accuracy_v1}"
...
echo "== Newton GL camera-view render =="
docker run --rm --gpus all \
  ...
  python3 tools/render_thread_robot_newton_gl.py \
    ...
    --height 720 \
    --fov 38 \
  2>&1 | tee "$OUT_NAME/newton_thread_robot_camera_log.txt"

echo "== Newton GL oblique render =="
docker run --rm --gpus all \
  -e PYOPENGL_PLATFORM=egl \
  -e PYTHONPATH=/workspace/thread_reconstruction:/workspace/thread_reconstruction/src \
  -v "$PWD:/workspace/thread_reconstruction" \
  ...
  python3 tools/render_thread_robot_newton_gl.py \
    ...
    --height 720 \
    --fov 38 \
  2>&1 | tee "$OUT_NAME/newton_thread_robot_oblique_log.txt"
